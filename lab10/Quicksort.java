...
/** Testing routines for quicksort.
...
class Quicksort {
    ...
    /** When run with ARGS N and K (java SortTesting N K), creates a pseudo-
     *  random permutation of the integers 0 .. N-1, uses quicksort to
    ...
    /** Assuming L <= U, return i such that L <= i <= U, attempting
    ...
    private static int pivot(int[] A, int L, int U, int kp) {
        /* Method ascribed to Nico Lomuto. The loop maintains the
         * invariant that L <= M < i <= U and A[L+1..M] are all < P,
